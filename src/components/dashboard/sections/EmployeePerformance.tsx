    ...
    <div className="mb-12">
      <CollapsibleSection section="Section 03" title="Employee Performance">
        ...
        <div className="bg-white border border-border/60 rounded-3xl p-6 shadow-sm">
          <div className="mb-4">
            <h3 className="font-bold text-slate-900">Top 5 Performers</h3>
            <p className="text-[11px] text-slate-500">Ranked by weighted productivity score</p>
          </div>
          <div className="space-y-3">
            {TOP_PERFORMERS.map((emp, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-100">
                <div className="flex items-center gap-3">
                  <div className="flex items-center justify-center w-6 h-6 rounded-full bg-white font-bold text-[11px] text-slate-400 shadow-sm">
                    {emp.isTop ? <Trophy className="h-3 w-3 text-amber-500" /> : i + 1}
                  </div>
                  <div>
                    <p className="text-[13px] font-bold text-slate-900 leading-tight">{emp.name}</p>
                    <p className="text-[11px] text-slate-500">{emp.dept}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white border border-border/60 rounded-3xl p-6 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-6">Late Coming Leaderboard</h3>
            <div className="space-y-4">
              {LATE_LEADERBOARD.map((emp, i) => (
                <div key={i} className="flex justify-between items-center">
                  <p className="text-[14px] text-slate-500 font-medium">{emp.name}</p>
          ...
          {/* Needs Attention */}
          <div className="bg-white border border-border/60 rounded-3xl p-6 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-6">Needs Attention</h3>
            <div className="space-y-6">
              {NEEDS_ATTENTION.map((emp, i) => (
                <div key={i}>
                  <div className="flex justify-between items-center mb-2">
                    <p className="text-[14px] text-slate-900">{emp.name}</p>
                  ...
                  <p className="text-[12px] text-slate-400">{emp.dept}</p>
                </div>
              ))}
            </div>
          </div>
