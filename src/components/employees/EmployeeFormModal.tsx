  ...
  return (
    <div className="fixed inset-0 z-[60] overflow-y-auto bg-card/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="flex min-h-full items-center justify-center p-4 text-center sm:p-6">
        ...
        >
          <div className="flex items-center justify-between px-8 py-6 border-b border-border/50 bg-muted/50/50">
            <div>
              <h2 className="text-xl font-black text-foreground">{initialData ? 'Edit Employee' : 'Add New Employee'}</h2>
              <p className="text-sm text-muted-foreground mt-1">{initialData ? 'Update employee details in the directory.' : 'Add a new member to your organization.'}</p>
            </div>
            <button 
              onClick={onClose}
